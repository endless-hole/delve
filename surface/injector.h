...
uintptr_t call_remote_load_library(std::shared_ptr< Driver > driver, DWORD thread_id, LPCSTR dll_name)
{
	HMODULE nt_dll = LoadLibraryW(L"ntdll.dll");

	uint64_t alloc_shell_code = driver->alloc(4096, PAGE_EXECUTE_READWRITE);
	...
	*(uintptr_t*)((uintptr_t)alloc_local + shell_data_offset) = shell_data;
	...
	driver->write_memory(alloc_shell_code, alloc_local, shell_size);
	...
	while (ll_data->status != 2) 
	{
		...
		driver->read_memory(shell_data, (PVOID)ll_data, sizeof(load_library_struct));
		Sleep(10);
	} uintptr_t mod_base = ll_data->module_base;

	UnhookWindowsHookEx(h_hook);
	driver->free(alloc_shell_code);
	VirtualFree(alloc_local, 0, MEM_RELEASE);

	return mod_base;
}

void call_dll_main(std::shared_ptr< Driver > driver, DWORD thread_id, uint64_t dll_base, PIMAGE_NT_HEADERS nt_header, bool hide_dll)
{
	HMODULE nt_dll = LoadLibraryW(L"ntdll.dll");
	...
	*(uintptr_t*)((uintptr_t)alloc_local + shell_data_offset) = shell_data;
	...
	main_data->fn_dll_main = (dll_base + nt_header->OptionalHeader.AddressOfEntryPoint);
	...
	{
		PostThreadMessage(thread_id, WM_NULL, 0, 0);
		driver->read_memory(shell_data, (PVOID)main_data, sizeof(main_struct));
		Sleep(10);
	}
...
PVOID rva_va( uintptr_t rva, PIMAGE_NT_HEADERS nt_head, PVOID local_image )
{
    ...
    for(
        PIMAGE_SECTION_HEADER p_section = p_first_sect;
...
bool relocate_image( void* remote_img, void* local_image, PIMAGE_NT_HEADERS nt_head )
{
    struct reloc_entry
    {
        uint32_t to_rva;
    ...
    uintptr_t delta_offset = ( uintptr_t )remote_img - nt_head->OptionalHeader.ImageBase;

    if( !delta_offset ) { return true; }
    if( !( nt_head->OptionalHeader.DllCharacteristics & IMAGE_DLLCHARACTERISTICS_DYNAMIC_BASE ) ) { return false; }

    reloc_entry* reloc_ent = ( reloc_entry* )rva_va
    (
        nt_head->OptionalHeader.DataDirectory[ IMAGE_DIRECTORY_ENTRY_BASERELOC ].VirtualAddress,
        nt_head,
        local_image
    );

    uintptr_t reloc_end = ( uintptr_t )reloc_ent + nt_head->OptionalHeader.DataDirectory[
    ...
    while ((uintptr_t)reloc_ent < reloc_end && reloc_ent->size)
	{
		...
		for (DWORD i = 0; i < records_count; i++)
		{
			...
			{
				uintptr_t fix_va = (uintptr_t)rva_va(reloc_ent->to_rva, nt_head, local_image);

				if (!fix_va)
					fix_va = (uintptr_t)local_image;
...
BOOL resolve_import(std::shared_ptr< Driver > driver, DWORD thread_id, PVOID p_local_img, PIMAGE_NT_HEADERS nt_head)
{
	...
	while ((module_name = (LPSTR)rva_va(import_desc->Name, nt_head, p_local_img)))
	{
		...
		base_image = call_remote_load_library(driver, thread_id, module_name);
...
void write_sections(std::shared_ptr< Driver > driver, uint64_t p_module_base, void* local_image, PIMAGE_NT_HEADERS nt_head)
{
	...
	{
		driver->write_memory( 
			(uint64_t)(p_module_base + section->VirtualAddress),
			(void*)((uintptr_t)local_image + section->PointerToRawData),
			section->SizeOfRawData 
		);
	}
}

void erase_discardable_sect(std::shared_ptr< Driver > driver, uint64_t p_module_base, PIMAGE_NT_HEADERS nt_head)
{
	PIMAGE_SECTION_HEADER section = IMAGE_FIRST_SECTION(nt_head);
	for (WORD sec_cnt = 0; sec_cnt < nt_head->FileHeader.NumberOfSections; sec_cnt++, section++)
	{
		...
		if (section->Characteristics & IMAGE_SCN_MEM_DISCARDABLE)
		{
			...
			driver->write_memory( (uint64_t)(p_module_base + section->VirtualAddress), zero_memory, section->SizeOfRawData );
			VirtualFree(zero_memory, 0, MEM_RELEASE);
		}
	}
}

bool injector( std::shared_ptr< Driver > driver, wchar_t* process_name, char* class_name, LPCWSTR dll_path )
{

    auto dll_image = get_dll_by_file( dll_path );

    if( !dll_image )
    {
        LOG( "Failed to find dll!" );
        return false;
    }

	Sleep(1000);

    PIMAGE_NT_HEADERS dll_nt_header = RtlImageNtHeader( dll_image );

    size_t dll_size = dll_nt_header->OptionalHeader.SizeOfImage;
    ...
    if( !driver->set_target( process_name ) )
    {
        LOG( "Failed to bind to target!" );
        return false;
    }
	...
	DWORD thread_id;
	DWORD process_id = get_process_id_and_thread_id_by_window_class(class_name, &thread_id);

	LOG( "process_id: ", (void*)process_id);
	LOG( "thread_id: ", (void*)thread_id);

	Sleep(1000);

	if(!thread_id || !process_id)
	{
		LOG("Failed to get thread id!");
		return false;
	}	
	
    uint64_t allocated_base = driver->alloc( dll_size, PAGE_EXECUTE_READWRITE );

    LOG( "allocated_base: 0x", (void*)allocated_base );

	Sleep(1000);

    if(!relocate_image( (void*)allocated_base, dll_image, dll_nt_header ))
    {
        driver->free( allocated_base );
        LOG("img relocation failed!");
    	return false;
    }
	...
	if (!resolve_import(driver, thread_id, dll_image, dll_nt_header))
	{
		driver->free( allocated_base );
        LOG("imports failed!");
    	return false;
	...
	write_sections(driver, allocated_base, dll_image, dll_nt_header);

	LOG("wrote sections!");

	Sleep(1000);

	call_dll_main(driver, thread_id, allocated_base, dll_nt_header, false);

	LOG("call dll main success!");

	Sleep(1000);

	erase_discardable_sect(driver, allocated_base, dll_nt_header);
