...
handle_t
proc_open(handle_t pid, ulong_t flags)
{
	handle_t h;
...
void*
proc_vmalloc(handle_t h, size_t size, ulong_t alloctype, ulong_t flags)
{
	void* p = 0;
...
ulong_t
proc_vmprotect(handle_t h, void* addr, size_t size, ulong_t flags)
{
	/*
...
size_t
proc_vmwrite(handle_t h, void* addr, void* buffer, size_t size)
{
	/* written bytes */
	size_t wr = 0;
...
#pragma optimize( "", off )
size_t
proc_vmread(handle_t h, void* addr, void* buffer, size_t size)
{
	/* read bytes */
	size_t rd = 0;
...
void
proc_vmreadpages(handle_t h, void* addr, void* buffer, size_t size)
{
...
size_t
proc_vmquery(handle_t h, void* addr, MEMORY_BASIC_INFORMATION* mbi, size_t size)
{
	/*
...
void
proc_get_peb(handle_t h, void** peb, void** peb_wow64)
{
	PROCESS_BASIC_INFORMATION pbi;

	/* null-out struct */
	mm_set(&pbi, 0, sizeof(pbi));
...
bool_t
thread_suspend(handle_t h, ulong_t* prev_susp_count)
{
...
bool_t
process_get_by_name(wchar_t* proc_name, handle_t* out_pid)
{
	NTSTATUS status;
	void* buffer = NULL;
	...
	while (spi->NextEntryOffset)
	{
		...
		if (!str_ucmp(proc_name, spi->ImageName.Buffer))
		{
...
void*
remote_get_module(handle_t proc_handle, char* mod_name, 
	void** remote_module_base, ulong_t proc_type)
{
	void* result;
	...
	char* mod_name, char* func_name, ulong_t ord, ulong_t code_type)
{
		...
		(uint8_t *)result < (uint8_t *)exp_dir + exp_size )
	{
		...
		str_acat((char *)tmp_mod_name, ".dll");
		str_acpy((char *)tmp_func_name, p + 1);

#if 0

		dbg_msg("forwarded export: %s, %s\n", tmp_mod_name, (char *)tmp_func_name);
...
void 
image_base_size(uint64_t* out_image_base, uint32_t* out_image_size)
{
	void* mod = (void*)proc_get_image_base();
	*out_image_base = pe64_get_image_base(mod);
	*out_image_size = pe64_get_image_size(mod);
}